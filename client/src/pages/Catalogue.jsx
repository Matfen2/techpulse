    ...
    <div>
      ...
      <div className="flex gap-2">
        <input
          ...
          placeholder="Min"
          className="w-full px-3 py-2.5 bg-[var(--bg-deep)] border border-[var(--border)] rounded-lg text-sm text-[var(--text)] placeholder-[var(--text-muted)]/50 focus:outline-none focus:border-[var(--orange)] font-mono transition-colors"
        />
...
const Catalogue = () => {
  ...
  return (
    <div className="max-w-8xl mx-auto px-4 py-6 sm:py-8">
      ...
      >
        <h1 className="text-2xl sm:text-3xl font-bold text-[var(--text)] mb-1">Catalogue</h1>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="hidden md:flex items-center -mt-20 justify-between mb-6">
              ...
              <AnimatePresence>
                {filters.category && (
                  <motion.span
                    ...
                    className="px-3 py-1 bg-[var(--orange)]/10 text-[var(--orange)] rounded-full text-xs flex items-center gap-1.5"
                  >
                ...
                {filters.brand && (
                  <motion.span
                    ...
                    animate={{ opacity: 1, scale: 1 }}
                    exit={{ opacity: 0, scale: 0.8 }}
                    className="px-3 py-1 bg-sky-500/10 text-sky-400 rounded-full text-xs flex items-center gap-1"
                  >
                    {filters.brand}
                ...
                {filters.search && (
                  <motion.span
                    ...
                    animate={{ opacity: 1, scale: 1 }}
                    exit={{ opacity: 0, scale: 0.8 }}
                    className="px-3 py-1 bg-purple-500/10 text-purple-400 rounded-full text-xs flex items-center gap-1"
                  >
                    « {filters.search} »
            ...
            <select
              value={filters.sort}
              onChange={(e) => setFilter('sort', e.target.value)}
              className="px-3 py-2 bg-[var(--bg-card)] border border-[var(--border)] rounded-lg text-sm text-[var(--text-muted)] focus:outline-none focus:border-[var(--orange)] cursor-pointer"
            >
              {sortOptions.map((opt) => (
                <option key={opt.value} value={opt.value}>{opt.label}</option>
              ))}
