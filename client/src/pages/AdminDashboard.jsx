...
const BRAND_COLORS = {
  Samsung: '#1428A0', Apple: '#A2AAAD', Xiaomi: '#FF6900',
  Sony: '#000000', Asus: '#00529B', Lenovo: '#E2231A',
};
const FALLBACK_COLORS = ['#f97316', '#06b6d4', '#8b5cf6', '#10b981', '#f43f5e', '#eab308'];
const STATUS_LABELS = { active: 'Actives', pending: 'En attente', sold: 'Vendues', rejected: 'Rejetées' };
const STATUS_COLORS = { active: '#10b981', pending: '#f59e0b', sold: '#06b6d4', rejected: '#ef4444' };
...
const Stars = ({ rating }) => {
  ...
  return (
    <span className="flex items-center gap-0.5">
      {[...Array(5)].map((_, i) => (
        <span key={i} className={`text-[10px] ${i < full ? 'text-amber-400' : 'text-[var(--border)]'}`}>★</span>
      ))}
    </span>
  );
};
...
const DashSkeleton = () => (
  <div className="min-h-screen bg-[var(--bg-deep)] py-6 sm:py-8 px-4 sm:px-6 lg:px-8">
    <div className="max-w-7xl mx-auto animate-pulse">
      <div className="h-7 w-48 bg-[var(--bg-card)] rounded mb-2" />
      <div className="h-4 w-32 bg-[var(--bg-card)] rounded mb-8" />
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-8">
        {Array.from({ length: 4 }).map((_, i) => (
          <div key={i} className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl h-28" />
        ))}
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-8">
        <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl h-64" />
        <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl h-64" />
      </div>
    </div>
  </div>
...
const AdminDashboard = () => {
  ...
  if (!stats) return (
    <div className="min-h-screen bg-[var(--bg-deep)] flex items-center justify-center px-4">
      <div className="text-center"><div className="text-4xl mb-3">⚠️</div><p className="text-red-400 text-sm">Erreur de chargement</p></div>
    </div>
  );

  const brandData = (stats.productsByBrand || []).map((b) => ({ ...b, color: BRAND_COLORS[b.name] || FALLBACK_COLORS[0] }));
  const categoryData = (stats.productsByCategory || []).map((c) => ({ name: c.name, Produits: c.count }));
  const statusData = (stats.listingsByStatus || []).map((s) => ({ name: STATUS_LABELS[s.name] || s.name, value: s.count, color: STATUS_COLORS[s.name] || '#6b7280' }));
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6 sm:mb-8">
          {[
            ...
            <motion.div key={kpi.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }} className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-3.5 sm:p-5">
        ...
        {stats.pendingListings > 0 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-6 p-3 sm:p-4 bg-amber-500/10 border border-amber-500/30 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-6 sm:mb-8">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-4 sm:p-6">
            ...
            {brandData.length > 0 ? (
              <ResponsiveContainer width="100%" height={200}>
                <PieChart>
                  <Pie data={brandData} cx="50%" cy="50%" innerRadius={45} outerRadius={75} paddingAngle={3} dataKey="count" nameKey="name">
                    {brandData.map((entry, idx) => <Cell key={entry.name} fill={entry.color || FALLBACK_COLORS[idx % FALLBACK_COLORS.length]} />)}
                  </Pie>
                  <Tooltip content={<CustomTooltip />} />
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-4 sm:p-6">
            ...
            {statusData.length > 0 ? (
              <ResponsiveContainer width="100%" height={170}>
                <PieChart>
                  <Pie data={statusData} cx="50%" cy="50%" innerRadius={35} outerRadius={60} paddingAngle={3} dataKey="value" nameKey="name">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.55 }} className="bg-[var(--bg-card)] border border-[var(--border)] rounded-xl p-4 sm:p-6">
            ...
            <div className="space-y-2.5">
              {[
                ...
                { label: 'Actives', value: stats.activeListings, color: 'text-emerald-400' },
            ...
            {stats.pendingListings > 0 && (
              <Link to="/admin/listings?status=pending" className="block mt-4 text-center py-2 bg-amber-500/10 text-amber-400 rounded-lg text-xs hover:bg-amber-500/20 transition-colors">
                Modérer →
              </Link>
            )}
          </motion.div>
